...
error_sim = function(x = "./data/demo_data/", file = "DAB053demo01.tag16.fastq", y = "./data/error_data/", err_rate = 0.15, substitutions = 0.34, insertions = 0.33, deletions = 0.33) {
  ...
  for (j in 1:length(vlseq_split)) {
    ...
    for (i in 1:n_base_repl) {
      if ((vlseq_split[[j]][rnd_n[i]] == rnd_b[i]) & (rnd_b[i] != "G")){
        ...
        insert = sample(c("A", "C", "T", "G"), size = 1)
  ...
  out = paste(out_file, out_err, "indel", out_sub, out_ins, out_del, "NEW", "fastq", sep = ".")
